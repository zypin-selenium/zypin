...
test('Requirements should be satisfied', async ({ ok, doesNotThrow }) => (
  ok((!existsSync(LOG_DIR) && mkdirSync(LOG_DIR, { recursive: true }), existsSync(LOG_DIR)), 'Log directory should exist'),
  doesNotThrow(() => execSync('java -version', { stdio: 'ignore' }), 'Java should be installed')
));

test('Grid should start successfully', async ({ ok, doesNotReject }) => {
  ...
  ok((gridLog = createWriteStream(join(LOG_DIR, 'selenium-grid.log'), { flags: 'a' }), gridChild.stdout.pipe(gridLog), gridChild.stderr.pipe(gridLog), process.on('exit', () => (gridChild && gridChild.kill('SIGTERM'), gridLog.end()))), 'Grid should setup logging and cleanup');
  ...
  )), 'Grid should complete startup');
});

test('Tunnel should start successfully', async ({ ok, doesNotReject }) => {
  ...
  ok((tunnelLog = createWriteStream(join(LOG_DIR, 'tunnel.log'), { flags: 'a' }), tunnelChild.stdout.pipe(tunnelLog), tunnelChild.stderr.pipe(tunnelLog), process.on('exit', () => (tunnelChild && tunnelChild.kill('SIGTERM'), tunnelLog.end()))), 'Tunnel should setup logging and cleanup');
  await doesNotReject(() => new Promise((resolve, reject) => (
  ...
  )), 'Tunnel should complete startup');
});
